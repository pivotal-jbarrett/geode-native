...
#define _GF_MG_EXCEPTION_ADD3(x) \
  { "apache::geode::client::" #x, gcnew CreateException2(x::Create) }
#define _GF_MG_EXCEPTION_ADD4(x, y) \
  { "apache::geode::client::" #y, gcnew CreateException2(x::Create) }
...
gc_ptr(Dictionary<gc_ptr(String), gc_ptr(CreateException2)>) GeodeException::Init() {
  ...
  gc_ptr(array<NameDelegatePair>) exNamesDelegates =
      gcnew array<NameDelegatePair>{_GF_MG_EXCEPTION_ADD3(AssertionException),
  ...
  Native2ManagedExMap = gcnew Dictionary<gc_ptr(String), gc_ptr(CreateException2)>();
  for (System::Int32 index = 0; index < exNamesDelegates->Length; index++) {
    Native2ManagedExMap[exNamesDelegates[index].m_name] = exNamesDelegates[index].m_delegate;
  }
  return Native2ManagedExMap;
}

gc_ptr(System::Exception) GeodeException::Get(const apache::geode::client::Exception& nativeEx) {
  gc_ptr(Exception) innerException = nullptr;
  try {
    std::rethrow_if_nested(nativeEx);
  } catch (const apache::geode::client::Exception& ex) {
    innerException = GeodeException::Get(ex);
  }
  gc_ptr(String) exName = marshal_as<gc_ptr(String)>(nativeEx.getName());
  gc_ptr(CreateException2) exDelegate;
  if (Native2ManagedExMap->TryGetValue(exName, exDelegate)) {
    return exDelegate(nativeEx, innerException);
  }
  gc_ptr(String) exMsg = marshal_as<gc_ptr(String)>(nativeEx.getMessage());
  if (exMsg->StartsWith(GeodeException::MgSysExPrefix)) {
    // Get the exception type
    gc_ptr(String) mgExStr = exMsg->Substring(GeodeException::MgSysExPrefix->Length);
    System::Int32 colonIndex = mgExStr->IndexOf(':');
    if (colonIndex > 0) {
      gc_ptr(String) mgExName = mgExStr->Substring(0, colonIndex)->Trim();
      // Try to load this class by reflection
      gc_ptr(Type) mgExType = Type::GetType(mgExName, false, true);
      if (mgExType != nullptr) {
        gc_ptr(System::Reflection::ConstructorInfo) cInfo =
    ...
    if (innerException == nullptr) {
      return gcnew GeodeException(exName + ": " + exMsg, gcnew GeodeException(GetStackTrace(nativeEx)));
    }
  }
