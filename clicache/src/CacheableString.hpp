...
/// </summary>
PUBLIC ref class CacheableString : public IDataSerializablePrimitive, public CacheableKey {
 public:
  /// <summary>
  ...
  /// </exception>
  CacheableString(gc_ptr(String) value);
  ...
  /// </exception>
  CacheableString(gc_ptr(array<Char>) value);
  ...
  /// </remarks>
  /// <param name="value">the string value of the new instance</param>
  inline static gc_ptr(CacheableString) Create(gc_ptr(String) value) {
    return (value != nullptr ? gcnew CacheableString(value, true) : nullptr);
  }
  ...
  property int8_t DsCode {
    virtual int8_t get() { return static_cast<int8_t>(m_type); }
  }

  property System::UInt64 ObjectSize { System::UInt64 get() override; }
  ...
  /// the character array value of the new instance
  /// </param>
  inline static gc_ptr(CacheableString) Create(gc_ptr(array<Char>) value) {
    return (value != nullptr && value->Length > 0 ? gcnew CacheableString(value, true) : nullptr);
  }
  ...
  /// </summary>
  virtual gc_ptr(String) ToString() override { return m_value; }
  ...
  /// </summary>
  virtual bool Equals(gc_ptr(Apache::Geode::Client::ICacheableKey) other) override;
  ...
  property gc_ptr(String) Value {
    inline gc_ptr(String) get() { return m_value; }
  }
  ...
  /// </summary>
  inline static operator gc_ptr(String)(gc_ptr(CacheableString) str) {
  ...
  property System::UInt32 Length {
    inline System::UInt32 get() { return m_value->Length; }
  }
  ...
  static gc_ptr(ISerializable) createDeserializableHuge() {
    return gcnew CacheableString(static_cast<int8_t>(DSCode::CacheableASCIIStringHuge));
  }

  static gc_ptr(ISerializable) createUTFDeserializable() {
    return gcnew CacheableString(static_cast<int8_t>(DSCode::CacheableString));
  }

  static gc_ptr(ISerializable) createUTFDeserializableHuge() {
  ...
  CacheableString(System::UInt32 type) : CacheableKey() { m_type = static_cast<DSCode>(type); }

 private:
  gc_ptr(String) m_value;
  DSCode m_type;
  int m_hashcode;

  CacheableString() : CacheableKey() { m_type = DSCode::CacheableASCIIString; }
  ...
  /// for arguments.
  /// </summary>
  CacheableString(gc_ptr(String) value, bool noParamCheck);

  /// <summary>
  /// Private constructor to create a CacheableString without checking
  /// for arguments.
  /// </summary>
  CacheableString(gc_ptr(array<Char>) value, bool noParamCheck);

  /// <summary>
  /// Private constructor to wrap a native object pointer
  /// </summary>
  /// <param name="nativeptr">The native object pointer</param>
  inline CacheableString(std::shared_ptr<apache::geode::client::CacheableString> nativeptr) : CacheableKey(nativeptr) {
    m_value = msclr::interop::marshal_as<gc_ptr(String)>(nativeptr->value());
