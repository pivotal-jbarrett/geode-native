...
GENERIC(class TPropKey, class TPropValue)
public
delegate void PropertyVisitorGeneric(TPropKey key, TPropValue value);
...
[Serializable]
    ...
    /// or an integer.
    /// </summary>
    PUBLIC ref class Properties sealed : public IDataSerializablePrimitive {
 public:
  /// <summary>
  /// Default constructor: returns an empty collection.
  /// </summary>
  inline Properties() : Properties(native::Properties::create()) {}
  ...
  void ForEach(gc_ptr(PropertyVisitorGeneric<TPropKey, TPropValue>) visitor);

  /// <summary>
  /// Return the number of entries in the collection.
  /// </summary>
  /// <returns>the number of entries</returns>
  property System::UInt32 Size { System::UInt32 get(); }
  ...
  void AddAll(gc_ptr(Properties<TPropKey, TPropValue>) other);
  ...
  void Load(gc_ptr(String) fileName);
  ...
  /// </returns>
  static gc_ptr(Properties<TPropKey, TPropValue>) Create(std::shared_ptr<native::Properties> nativeptr) {
    return __nullptr == nativeptr ? nullptr : gcnew Properties<TPropKey, TPropValue>(nativeptr);
  }

  std::shared_ptr<native::Properties> GetNative() { return m_nativeptr->get_shared_ptr(); }

  inline static gc_ptr(ISerializable) CreateDeserializable() { return Create(); }
 ...
 private:
  ...
  gc_ptr(native_shared_ptr<native::Properties>) m_nativeptr;
 ...
 public:
  void Visit(gc_ptr(ICacheableKey) key, gc_ptr(ISerializable) value) {
    auto tpkey = TypeRegistry::GetManagedValueGeneric<TPropKey>(
        std::shared_ptr<apache::geode::client::Serializable>(GetNativeWrapperForManagedObject(key)));
    auto tpvalue = TypeRegistry::GetManagedValueGeneric<TPropValue>(
        std::shared_ptr<apache::geode::client::Serializable>(GetNativeWrapperForManagedObject(value)));
