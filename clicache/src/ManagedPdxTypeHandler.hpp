...
class ManagedPdxTypeHandler : public PdxTypeHandler {
 public:
  ...
  void serialize(const std::shared_ptr<PdxSerializable>& pdxSerializable, DataOutput& dataOutput) const override {
  ...
  std::shared_ptr<PdxSerializable> deserialize(DataInput& dataInput) const override {
    try {
      ...
      auto serializationRegistry =
          CacheRegionHelper::getCacheImpl(dataInput.getCache())->getSerializationRegistry().get();
      auto managedPdx =
          Apache::Geode::Client::Internal::PdxHelper::DeserializePdx(% managedDataInput, false, serializationRegistry);
