...
PUBLIC ref class CacheableHashSetType : public IDataSerializablePrimitive,
                                        public Serializable,
                                        public ICollection<gc_ptr(Object)> {
 public:
  virtual void ToData(gc_ptr(DataOutput) output) {
    ...
    for (const auto& iter : *set) {
      auto key = TypeRegistry::GetManagedValueGeneric<gc_ptr(Object)>(iter);
  ...
  virtual void FromData(gc_ptr(DataInput) input) {
    ...
    if (len > 0) {
      for (int i = 0; i < len; i++) {
        gc_ptr(Object) key = (input->ReadObject());
        this->Add(key);
      }
    }
  }
  ...
  property System::UInt64 ObjectSize {
    System::UInt64 get() override {
      ...
      FOR_EACH (gc_ptr(Object) key in this) {
        if (key != nullptr)
          // size += key->ObjectSize;
  ...
  bool Equals(gc_ptr(Object) other) override {
    if (other == nullptr) return false;

    gc_ptr(CacheableHashSetType) otherCHST = dynamic_cast<gc_ptr(CacheableHashSetType)>(other);

    if (otherCHST == nullptr) return false;
  ...
  /// </summary>
  ref class Enumerator sealed : public IEnumerator<gc_ptr(Object)> {
   public:
    // Region: IEnumerator<gc_ptr(ICacheableKey)> Members
    ...
    /// of the enumerator.
    /// </returns>
    property gc_ptr(Object) Current {
      virtual gc_ptr(Object) get() = IEnumerator<gc_ptr(Object)>::Current::get {
        if (!m_started) {
          throw gcnew System::InvalidOperationException("Call MoveNext first.");
        }
        auto ret = TypeRegistry::GetManagedValueGeneric<gc_ptr(Object)>(*(*(m_nativeptr->get())));
        GC::KeepAlive(this);
        return ret;
      }
    }

    // End Region: IEnumerator<gc_ptr(ICacheableKey)> Members
    ...
    virtual void Reset() {
      try {
        m_nativeptr = gcnew native_unique_ptr<typename HSTYPE::iterator>(
            std::make_unique<typename HSTYPE::iterator>(dynamic_cast<HSTYPE*>(m_set->m_nativeptr->get())->begin()));
      } finally {
   ...
   private:
    ...
    /// </exception>
    property gc_ptr(Object) ICurrent {
      virtual gc_ptr(Object) get() sealed = System::Collections::IEnumerator::Current::get { return Current; }
    }
    ...
    gc_ptr(CacheableHashSetType<TYPEID, HSTYPE>) m_set;

    gc_ptr(native_unique_ptr<typename HSTYPE::iterator>) m_nativeptr;
  };
  ...
  /// </param>
  virtual void Swap(gc_ptr(CacheableHashSetType<TYPEID, HSTYPE>) other) sealed {
    try {
      if (other != nullptr) {
        dynamic_cast<HSTYPE*>(m_nativeptr->get())->swap(*dynamic_cast<HSTYPE*>(other->m_nativeptr->get()));
      }
    } finally {
      GC::KeepAlive(m_nativeptr);
  ...
  // Region: ICollection<gc_ptr(ICacheableKey)> Members
  ...
  /// </param>
  virtual void Add(gc_ptr(Object) item) {
  ...
  /// otherwise false.
  /// </returns>
  virtual bool Contains(gc_ptr(Object) item) {
    try {
      return dynamic_cast<HSTYPE*>(m_nativeptr->get())->find(Serializable::GetUnmanagedValueGeneric(item)) !=
             dynamic_cast<HSTYPE*>(m_nativeptr->get())->end();
    } finally {
      GC::KeepAlive(m_nativeptr);
    }
  }
  ...
  virtual void CopyTo(gc_ptr(array<Object ^>) array, System::Int32 arrayIndex) {
    ...
    if (arrayIndex >= array->Length || array->Length < (arrayIndex + (System::Int32)set->size())) {
      throw gcnew OutOfRangeException(
          "CacheableHashSet.CopyTo():"
    ...
    for (const auto& iter : *set) {
      array[index++] = TypeRegistry::GetManagedValueGeneric<gc_ptr(Object)>(iter);
    }
  ...
  /// <c>CacheableHashSet</c>.
  /// </returns>
  virtual bool Remove(gc_ptr(Object) item) {
    try {
  ...
  // End Region: ICollection<gc_ptr(ICacheableKey)> Members

  // Region: IEnumerable<gc_ptr(ICacheableKey)> Members
 ...
 private:
  ...
  virtual gc_ptr(System::Collections::IEnumerator)
      GetIEnumerator() sealed = System::Collections::IEnumerable::GetEnumerator {
 ...
 protected:
  ...
  /// </summary>
  inline CacheableHashSetType<TYPEID, HSTYPE>() : Serializable(HSTYPE::createDeserializable()) {}

  /// <summary>
  /// Allocates a new empty instance with given initial size.
  /// </summary>
  /// <param name="size">The initial size of the HashSet.</param>
  inline CacheableHashSetType<TYPEID, HSTYPE>(System::Int32 size) : Serializable(HSTYPE::create(size)) {}
};
}  // namespace Internal
...
/// a distributable object for caching.
/// </summary>
_GFCLI_CACHEABLEHASHSET_DEF_GENERIC(CacheableHashSet, apache::geode::client::CacheableHashSet);
...
/// </summary>
_GFCLI_CACHEABLEHASHSET_DEF_GENERIC(CacheableLinkedHashSet, apache::geode::client::CacheableLinkedHashSet);
}  // namespace Client
}  // namespace Geode
}  // namespace Apache
