...
gc_ptr(String) PdxInstanceImpl::GetClassName() {
  if (m_typeId != 0) {
    gc_ptr(PdxType) pdxtype = m_cache->GetPdxTypeRegistry()->GetPdxType(m_typeId);
...
gc_ptr(Object) PdxInstanceImpl::GetObject() {
  ...
  gc_ptr(Object) ret = Internal::PdxHelper::DeserializePdx(
      dataInput, true, m_typeId, m_bufferLength,
      CacheRegionHelper::getCacheImpl(m_cache->GetNative().get())->getSerializationRegistry().get());

  if (m_cachePerfStats) {
    Utils::updateStatOpTime(m_cachePerfStats->getStat(), m_cachePerfStats->getPdxInstanceDeserializationTimeId(),
                            sampleStartNanos);
...
bool PdxInstanceImpl::HasField(gc_ptr(String) fieldName) {
...
bool PdxInstanceImpl::IsIdentityField(gc_ptr(String) fieldName) {
  gc_ptr(PdxType) pt = getPdxType();
  gc_ptr(PdxFieldType) pft = pt->GetPdxField(fieldName);

  return pft != nullptr && pft->IdentityField;
}

gc_ptr(Object) PdxInstanceImpl::GetField(gc_ptr(String) fieldName) {
  gc_ptr(PdxType) pt = getPdxType();
...

  {
    ...
    gc_ptr(Object) tmp = this->readField(dataInput, fieldName, pft->TypeId);
...
void PdxInstanceImpl::setOffsetForObject(gc_ptr(DataInput) dataInput, gc_ptr(PdxType) pt, int sequenceId) {
  int pos = getOffset(dataInput, pt, sequenceId);
  dataInput->ResetAndAdvanceCursorPdx(pos);
}

int PdxInstanceImpl::getOffset(gc_ptr(DataInput) dataInput, gc_ptr(PdxType) pt, int sequenceId) {
  dataInput->ResetPdx(0);
...
int PdxInstanceImpl::getSerializedLength(gc_ptr(DataInput) dataInput, gc_ptr(PdxType) pt) {
  dataInput->ResetPdx(0);
...
bool PdxInstanceImpl::Equals(gc_ptr(Object) other) {
  if (other == nullptr) return false;

  gc_ptr(PdxInstanceImpl) otherPdx = dynamic_cast<gc_ptr(PdxInstanceImpl)>(other);

  if (otherPdx == nullptr) return false;

  gc_ptr(PdxType) myPdxType = getPdxType();
  gc_ptr(PdxType) otherPdxType = otherPdx->getPdxType();

  if (!otherPdxType->PdxClassName->Equals(myPdxType->PdxClassName)) return false;
  ...
  gc_ptr(DataInput) myDataInput = gcnew DataInput(m_buffer, m_bufferLength, m_cache);
  myDataInput->setPdxdeserialization(true);
  gc_ptr(DataInput) otherDataInput = gcnew DataInput(otherPdx->m_buffer, otherPdx->m_bufferLength, m_cache);
  otherDataInput->setPdxdeserialization(true);

  bool isEqual = false;
  int fieldTypeId = -1;
  for (int i = 0; i < myPdxIdentityFieldList->Count; i++) {
    gc_ptr(PdxFieldType) myPFT = myPdxIdentityFieldList[i];
    gc_ptr(PdxFieldType) otherPFT = otherPdxIdentityFieldList[i];
    ...
    if (myPFT == Default_PdxFieldType) {
      ...
      /*gc_ptr(Object) val = otherPdx->GetField(otherPFT->FieldName);
      if(val == nullptr || (int)val == 0 || (bool)val == false)
      continue;*/
    } else if (otherPFT == Default_PdxFieldType) {
      fieldTypeId = myPFT->TypeId;
      /*gc_ptr(Object) val = this->GetField(myPFT->FieldName);
      if(val == nullptr || (int)val == 0 || (bool)val == false)
      continue;*/
    } else {
      fieldTypeId = myPFT->TypeId;
    ...
    switch (fieldTypeId) {
      ...
      case PdxFieldTypes::OBJECT: {
        gc_ptr(Object) object = nullptr;
        gc_ptr(Object) otherObject = nullptr;
        if (myPFT != Default_PdxFieldType) {
      ...
      case PdxFieldTypes::OBJECT_ARRAY: {
        gc_ptr(Object) objectArray = nullptr;
        gc_ptr(Object) otherObjectArray = nullptr;
...
bool PdxInstanceImpl::compareRawBytes(gc_ptr(PdxInstanceImpl) other, gc_ptr(PdxType) myPT, gc_ptr(PdxFieldType) myF,
  ...
  if (myF != Default_PdxFieldType && otherF != Default_PdxFieldType) {
    ...
    gc_ptr(DataInput) tmpDI = nullptr;
...
void PdxInstanceImpl::equatePdxFields(gc_ptr(IList<PdxFieldType ^>) my, gc_ptr(IList<PdxFieldType ^>) other) {
  ...
  for (int i = 0; i < my->Count; i++) {
    ...
    if (myF != Default_PdxFieldType) {
      ...
      {
        if (i < other->Count) {
          gc_ptr(PdxFieldType) tmp = other[i];
  ...
  //  {
  //    gc_ptr(PdxFieldType) otherF = other[i];
...
bool PdxInstanceImpl::deepArrayEquals(gc_ptr(Object) obj, gc_ptr(Object) otherObj) {
  ...
  gc_ptr(Type) objT = obj->GetType();
  gc_ptr(Type) otherObjT = otherObj->GetType();
  if (!objT->Equals(otherObjT)) return false;

  if (objT->IsArray) {  // array
    return enumerableEquals((gc_ptr(System::Collections::IEnumerable))obj,
                            (gc_ptr(System::Collections::IEnumerable))otherObj);
  } else if (objT->GetInterface("System.Collections.IDictionary")) {  // map
    // Log::Debug(" in map");
    return enumerateDictionaryForEqual((gc_ptr(System::Collections::IDictionary))obj,
                                       (gc_ptr(System::Collections::IDictionary))otherObj);
...
bool PdxInstanceImpl::enumerableEquals(gc_ptr(System::Collections::IEnumerable) enumObj,
  ...
  gc_ptr(System::Collections::IEnumerator) my = enumObj->GetEnumerator();
  gc_ptr(System::Collections::IEnumerator) other = enumOtherObj->GetEnumerator();
...
bool PdxInstanceImpl::enumerateDictionaryForEqual(gc_ptr(System::Collections::IDictionary) iDict,
  ...
  gc_ptr(System::Collections::IDictionaryEnumerator) dEnum = iDict->GetEnumerator();
  FOR_EACH (gc_ptr(System::Collections::DictionaryEntry) de in iDict) {
    gc_ptr(Object) other = nullptr;
    if (otherIDict->Contains(de->Key)) {
      if (!deepArrayEquals(de->Value, otherIDict[de->Key])) return false;
    } else
      return false;
  }
...
int PdxInstanceImpl::GetHashCode() {
  ...
  for (int i = 0; i < pdxIdentityFieldList->Count; i++) {
    gc_ptr(PdxFieldType) pField = pdxIdentityFieldList[i];
    ...
    switch (pField->TypeId) {
      ...
      case PdxFieldTypes::OBJECT: {
        ...
        gc_ptr(Object) object = dataInput->ReadObject();
      ...
      case PdxFieldTypes::OBJECT_ARRAY: {
        setOffsetForObject(dataInput, pt, pField->SequenceId);
        gc_ptr(Object) objectArray = dataInput->ReadObjectArray();
...
int PdxInstanceImpl::deepArrayHashCode(gc_ptr(Object) obj) {
  if (obj == nullptr) return 0;

  gc_ptr(Type) objT = obj->GetType();

  /*FOR_EACH (gc_ptr(Type) tmp in objT->GetInterfaces())
    //Log::Debug("interfaces " + tmp);*/

  if (objT->IsArray) {  // array
    // if(objT->GetElementType()->IsPrimitive)
    //{//primitive array
    //  return primitiveArrayHashCode((gc_ptr(array<int>))obj);
    //}
    // else
    {  // object array
      return enumerableHashCode((gc_ptr(System::Collections::IEnumerable))obj);
    }
  } else if (objT->GetInterface("System.Collections.IDictionary")) {  // map
    // Log::Debug(" in map");
    return enumerateDictionary((gc_ptr(System::Collections::IDictionary))obj);
  } else if (objT->GetInterface("System.Collections.IList")) {  // list
    // Log::Debug(" in list");
    return enumerableHashCode((gc_ptr(System::Collections::IEnumerable))obj);
  } else {
    ...
    if (obj->GetType()->Equals(DotNetTypes::BooleanType)) {
      ...
      gc_ptr(String) str = (gc_ptr(String))obj;
      int prime = 31;
      int h = 0;
...
int PdxInstanceImpl::enumerableHashCode(gc_ptr(System::Collections::IEnumerable) enumObj) {
  int h = 1;
  FOR_EACH (gc_ptr(Object) o in enumObj) {
...
int PdxInstanceImpl::enumerateDictionary(gc_ptr(System::Collections::IDictionary) iDict) {
  int h = 0;
  gc_ptr(System::Collections::IDictionaryEnumerator) dEnum = iDict->GetEnumerator();
  FOR_EACH (gc_ptr(System::Collections::DictionaryEntry) de in iDict) {
    // gc_ptr(System::Collections::DictionaryEntry) de = (gc_ptr(System::Collections::DictionaryEntry))o;
...
generic<class T> int PdxInstanceImpl::primitiveArrayHashCode(T objArray) {
  ...
  int h = 1;
  FOR_EACH (gc_ptr(Object) o in objArray) {
    if (isBooleanType) {
      if ((bool)o)
        h = h * 31 + 1231;
      else
...
int PdxInstanceImpl::getRawHashCode(gc_ptr(PdxType) pt, gc_ptr(PdxFieldType) pField, gc_ptr(DataInput) dataInput) {
  ...
  if (hasDefaultBytes(pField, dataInput, pos, nextpos)) return 0;  // matched default bytes
...
bool PdxInstanceImpl::compareDefaulBytes(gc_ptr(DataInput) dataInput, int start, int end,
                                         gc_ptr(array<SByte>) defaultBytes) {
...
bool PdxInstanceImpl::isPrimitiveArray(gc_ptr(Object) object) {
  gc_ptr(Type) type = object->GetType();
...
int PdxInstanceImpl::comparePdxField(gc_ptr(PdxFieldType) a, gc_ptr(PdxFieldType) b) {
  return a->FieldName->CompareTo(b->FieldName);
}

gc_ptr(String) PdxInstanceImpl::ToString() {
  gc_ptr(PdxType) pt = getPdxType();

  gc_ptr(StringBuilder) result = gcnew StringBuilder();
  result->Append("PDX[")->Append(pt->TypeId)->Append(",")->Append(pt->PdxClassName)->Append("]{");
  bool firstElement = true;
  FOR_EACH (gc_ptr(PdxFieldType) fieldType in getIdentityPdxFields(pt)) {
    if (firstElement) {
      firstElement = false;
    } else {
      result->Append(", ");
    ...
    } catch (gc_ptr(System::Exception) e) {
...
gc_ptr(IWritablePdxInstance) PdxInstanceImpl::CreateWriter() {
  // dataInput->ResetPdx(0);
  return gcnew PdxInstanceImpl(m_buffer, m_bufferLength, m_typeId, false,
                               m_cache);  // need to create duplicate byte stream
}

void PdxInstanceImpl::SetField(gc_ptr(String) fieldName, gc_ptr(Object) value) {
  gc_ptr(PdxType) pt = getPdxType();
  gc_ptr(PdxFieldType) pft = pt->GetPdxField(fieldName);
  ...
  if (pft != nullptr && checkType(value->GetType(), pft->TypeId))  // TODO::need to check typeas well
  {
    if (m_updatedFields == nullptr) {
      m_updatedFields = gcnew Dictionary<gc_ptr(String), gc_ptr(Object)>();
    }
    m_updatedFields[fieldName] = value;
    return;
  }

  throw gcnew IllegalStateException("PdxInstance doesn't has field " + fieldName + " or type of field not matched " +
...
void PdxInstanceImpl::ToData(gc_ptr(IPdxWriter) writer) {
  ...
  if (m_buffer != NULL) {
    ...
    if (!m_own) copy = apache::geode::client::DataInput::getBufferCopy(m_buffer, m_bufferLength);

    gc_ptr(DataInput) dataInput = gcnew DataInput(copy, m_bufferLength, m_cache);  // this will delete buffer
    dataInput->setPdxdeserialization(true);
    ...
    for (int i = 0; i < pdxFieldList->Count; i++) {
      ...
      if (value != nullptr) {  //
        ...
        if (currPf->IsVariableLengthType) {  // need to add offset
          (static_cast<gc_ptr(PdxLocalWriter)>(writer))->AddOffset();
        }

        // write raw byte array...
        nextFieldPosition = getNextFieldPosition(dataInput, i + 1, pt);

        writeUnmodifieldField(dataInput, position, nextFieldPosition, static_cast<gc_ptr(PdxLocalWriter)>(writer));
    ...
    for (int i = 0; i < pdxFieldList->Count; i++) {
      ...
      gc_ptr(Object) value = m_updatedFields[currPf->FieldName];
...
void PdxInstanceImpl::writeUnmodifieldField(gc_ptr(DataInput) dataInput, int startPos, int endPos,
...
int PdxInstanceImpl::getNextFieldPosition(gc_ptr(DataInput) dataInput, int fieldId, gc_ptr(PdxType) pt) {
...
void PdxInstanceImpl::FromData(gc_ptr(IPdxReader) reader) {
  throw gcnew IllegalStateException("PdxInstance::FromData( .. ) shouldn't have called");
}

gc_ptr(PdxType) PdxInstanceImpl::getPdxType() {
  if (m_typeId == 0) {
  ...
  gc_ptr(PdxType) pType = m_cache->GetPdxTypeRegistry()->GetPdxType(m_typeId);
...
bool PdxInstanceImpl::checkType(gc_ptr(Type) type, int typeId) {
...
void PdxInstanceImpl::writeField(gc_ptr(IPdxWriter) writer, gc_ptr(String) fieldName, int typeId,
                                 gc_ptr(Object) value) {
  switch (typeId) {
    case PdxFieldTypes::INT: {
    ...
    case PdxFieldTypes::STRING: {
      writer->WriteString(fieldName, (gc_ptr(String))value);
      break;
    }
    case PdxFieldTypes::BOOLEAN: {
    ...
    case PdxFieldTypes::BYTE_ARRAY: {
      writer->WriteByteArray(fieldName, (gc_ptr(array<Byte>))value);
      break;
    }
    case PdxFieldTypes::DOUBLE_ARRAY: {
      writer->WriteDoubleArray(fieldName, (gc_ptr(array<double>))value);
      break;
    }
    case PdxFieldTypes::FLOAT_ARRAY: {
      writer->WriteFloatArray(fieldName, (gc_ptr(array<float>))value);
      break;
    }
    case PdxFieldTypes::SHORT_ARRAY: {
      writer->WriteShortArray(fieldName, (gc_ptr(array<short>))value);
      break;
    }
    case PdxFieldTypes::INT_ARRAY: {
      writer->WriteIntArray(fieldName, (gc_ptr(array<int>))value);
      break;
    }
    case PdxFieldTypes::LONG_ARRAY: {
      writer->WriteLongArray(fieldName, (gc_ptr(array<Int64>))value);
      break;
    }
    case PdxFieldTypes::BOOLEAN_ARRAY: {
      writer->WriteBooleanArray(fieldName, (gc_ptr(array<bool>))value);
      break;
    }
    case PdxFieldTypes::CHAR_ARRAY: {
      writer->WriteCharArray(fieldName, (gc_ptr(array<Char>))value);
      break;
    }
    case PdxFieldTypes::STRING_ARRAY: {
    ...
    case PdxFieldTypes::ARRAY_OF_BYTE_ARRAYS: {
      writer->WriteArrayOfByteArrays(fieldName, (array<gc_ptr(array<Byte>)> ^) value);
      break;
    }
    case PdxFieldTypes::OBJECT_ARRAY: {
