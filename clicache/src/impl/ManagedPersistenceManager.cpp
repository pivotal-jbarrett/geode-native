...
apache::geode::client::PersistenceManager* ManagedPersistenceManagerGeneric::create(const char* assemblyPath,
  ...
  try {
    ...
    if (typeInst != nullptr) {
      ...
      if (mInfo != nullptr) {
        ...
        if (managedptr == nullptr) {
          std::string ex_str =
              "ManagedPersistenceManagerGeneric: Could not create "
        ...
        gc_ptr(Type) clgType = Type::GetType("Apache.Geode.Client.PersistenceManagerGeneric`2");
        ...
        mgcl->setptr((gc_ptr(Apache::Geode::Client::IPersistenceManagerProxy))clg);

        return mgcl;
      } else {
        std::string ex_str =
            "ManagedPersistenceManagerGeneric: Could not load "
            "function with name [";
        ex_str += factoryFunctionName;
        ex_str += "] in assembly: ";
        ex_str += assemblyPath;
    ...
    std::string ex_str =
        "ManagedPersistenceManagerGeneric: Got an exception while "
...
void ManagedPersistenceManagerGeneric::write(const std::shared_ptr<CacheableKey>& key,
                                             const std::shared_ptr<Cacheable>& value, std::shared_ptr<void>&) {
  m_managedptr->write(key, value);
}

bool ManagedPersistenceManagerGeneric::writeAll() { throw gcnew System::NotSupportedException; }

void ManagedPersistenceManagerGeneric::init(const std::shared_ptr<Region>& region,
                                            const std::shared_ptr<Properties>& diskProperties) {
  m_managedptr->init(region, diskProperties);
}

std::shared_ptr<Cacheable> ManagedPersistenceManagerGeneric::read(const std::shared_ptr<CacheableKey>& key,
                                                                  const std::shared_ptr<void>&) {
  return m_managedptr->read(key);
}

bool ManagedPersistenceManagerGeneric::readAll() { throw gcnew System::NotSupportedException; }
...
void ManagedPersistenceManagerGeneric::close() { m_managedptr->close(); }
}  // namespace client
}  // namespace geode
}  // namespace apache
