...
CacheWriter* ManagedCacheWriterGeneric::create(const char* assemblyPath, const char* factoryFunctionName) {
  try {
    ...
    if (typeInst != nullptr) {
      ...
      if (mInfo != nullptr) {
        ...
        if (managedptr == nullptr) {
          std::string ex_str =
              "ManagedCacheWriterGeneric: Could not create "
        ...
        gc_ptr(Type) cwgType = Type::GetType("Apache.Geode.Client.CacheWriterGeneric`2");
        ...
        std::string ex_str =
            "ManagedCacheWriterGeneric: Could not load "
            "function with name [";
        ex_str += factoryFunctionName;
        ex_str += "] in assembly: ";
        ex_str += assemblyPath;
    ...
    std::string ex_str =
        "ManagedCacheWriterGeneric: Got an exception while "
...
void ManagedCacheWriterGeneric::close(const std::shared_ptr<Region>& rp) {
  try {
    auto mregion = Apache::Geode::Client::Region<gc_ptr(Object), gc_ptr(Object)>::Create(rp);
