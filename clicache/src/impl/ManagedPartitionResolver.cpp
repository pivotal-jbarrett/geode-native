...
PartitionResolver* ManagedPartitionResolverGeneric::create(const char* assemblyPath, const char* factoryFunctionName) {
  try {
    ...
    if (typeInst != nullptr) {
      ...
      if (mInfo != nullptr) {
        ...
        if (managedptr == nullptr) {
          std::string ex_str =
              "ManagedPartitionResolverGeneric: Could not create "
        ...
        gc_ptr(Type) prgType = Type::GetType("Apache.Geode.Client.PartitionResolverGeneric`2");
        ...
        mgpr->setptr((gc_ptr(Apache::Geode::Client::IPartitionResolverProxy))prg);

        return mgpr;
      } else {
        std::string ex_str =
            "ManagedPartitionResolverGeneric: Could not load "
            "function with name [";
        ex_str += factoryFunctionName;
        ex_str += "] in assembly: ";
        ex_str += assemblyPath;
    ...
    std::string ex_str =
        "ManagedPartitionResolverGeneric: Got an exception while "
