...
void PdxHelper::SerializePdx(gc_ptr(DataOutput) dataOutput, gc_ptr(IPdxSerializable) pdxObject) {
  dataOutput->setPdxSerialization(true);
  gc_ptr(String) pdxClassname = nullptr;
  bool isPdxWrapper = false;
  gc_ptr(Type) pdxType = nullptr;

  if (auto pdxWrapper = dynamic_cast<gc_ptr(PdxWrapper)>(pdxObject)) {
    isPdxWrapper = true;
    pdxType = pdxWrapper->GetObject()->GetType();
  } else {
    if (auto pdxII = dynamic_cast<gc_ptr(PdxInstanceImpl)>(pdxObject)) {
      gc_ptr(PdxType) piPt = pdxII->getPdxType();
      ...
      gc_ptr(PdxLocalWriter) plw = gcnew PdxLocalWriter(dataOutput, piPt);
      pdxII->ToData(plw);
  ...
  gc_ptr(PdxType) localPdxType = dataOutput->Cache->GetPdxTypeRegistry()->GetLocalPdxType(pdxClassname);

  if (localPdxType == nullptr) {
    // need to grab type info, as fromdata is not called yet
    gc_ptr(PdxWriterWithTypeCollector) ptc = gcnew PdxWriterWithTypeCollector(dataOutput, pdxClassname);
    ...
    // + 1 + 2*4); //pdxLen + 93 DSID + len + typeID GC::KeepAlive(dataOutput);
  } else  // we know locasl type, need to see preerved data
  {
    ...
    gc_ptr(PdxRemoteWriter) prw = nullptr;
    if (pd != nullptr) {
      gc_ptr(PdxType) mergedPdxType = cache->GetPdxTypeRegistry()->GetPdxType(pd->MergedTypeId);
...
gc_ptr(IPdxSerializable) PdxHelper::DeserializePdx(gc_ptr(DataInput) dataInput, bool forceDeserialize, int typeId,
  ...
  gc_ptr(IPdxSerializable) pdxObject = nullptr;
  ...
  gc_ptr(PdxType) pType = cache->GetPdxTypeRegistry()->GetPdxType(typeId);
  gc_ptr(PdxType) pdxLocalType = nullptr;

  if (pType != nullptr)  // this may happen with PdxInstanceFactory
    pdxLocalType =
        cache->GetPdxTypeRegistry()->GetLocalPdxType(pType->PdxClassName);  // this should be fine for IPdxTypeMapper
  ...
  if (pType != nullptr && pdxLocalType != nullptr)  // type found
  {
    ...
    {
      gc_ptr(PdxLocalReader) plr = gcnew PdxLocalReader(dataInput, pType, len);
      pdxObject->FromData(plr);
      plr->MoveStream();  // it will set stream
    } else {
      gc_ptr(PdxRemoteReader) prr = gcnew PdxRemoteReader(dataInput, pType, len);
      pdxObject->FromData(prr);

      gc_ptr(PdxType) mergedVersion = cache->GetPdxTypeRegistry()->GetMergedType(pType->TypeId);
      gc_ptr(PdxRemotePreservedData) preserveData = prr->GetPreservedData(mergedVersion, pdxObject);
  ...
  {
    if (pType == nullptr) {
      pType = (gc_ptr(PdxType))(Serializable::GetPDXTypeById(dataInput->GetPool(), typeId, dataInput->Cache));
      pdxLocalType = dataInput->Cache->GetPdxTypeRegistry()->GetLocalPdxType(
          pType->PdxClassName);  // this should be fine for IPdxTypeMappers
    }
    ...
    gc_ptr(Object) pdxRealObject = pdxObject;
    bool isPdxWrapper = false;
    ...
    {
      gc_ptr(PdxReaderWithTypeCollector) prtc = gcnew PdxReaderWithTypeCollector(dataInput, pType, len);
      pdxObject->FromData(prtc);

      if (isPdxWrapper) pdxRealObject = pdxWrapper->GetObject();
      ...
      if (pType->Equals(pdxLocalType))  // same
      {
        ...
        pdxLocalType->TypeId = dataInput->Cache->GetPdxTypeRegistry()->GetPDXIdForType(
            pdxObject->GetType(), dataInput->GetPool(), pdxLocalType, true);
        ...
        gc_ptr(PdxType) mergedVersion = dataInput->Cache->GetPdxTypeRegistry()->GetMergedType(pType->TypeId);
        gc_ptr(PdxRemotePreservedData) preserveData = prtc->GetPreservedData(mergedVersion, pdxObject);
        if (preserveData != nullptr) dataInput->Cache->GetPdxTypeRegistry()->SetPreserveData(pdxObject, preserveData);
      }
      prtc->MoveStream();
...
gc_ptr(IPdxSerializable) PdxHelper::DeserializePdx(gc_ptr(DataInput) dataInput, bool forceDeserialize,
  ...
  try {
    ...
    if (dataInput->Cache->GetPdxReadSerialized() == false || forceDeserialize || dataInput->isRootObjectPdx()) {
      ...
      // This is for pdx Statistics
      CacheRegionHelper::getCacheImpl(dataInput->GetNative()->getCache())
          ->getCachePerfStats()
          .incPdxDeSerialization(len + 9);  // pdxLen + 1 + 2*4
      ...
      gc_ptr(PdxType) pType = dataInput->Cache->GetPdxTypeRegistry()->GetPdxType(typeId);

      if (pType == nullptr) {
        gc_ptr(PdxType) pType =
            (gc_ptr(PdxType))(Serializable::GetPDXTypeById(dataInput->GetPool(), typeId, dataInput->Cache));
      ...
      // pdxObject = gcnew PdxInstanceImpl(gcnew DataInput(dataInput->GetBytes(dataInput->GetCursor(), len  + 8 ), len
      // + 8));
      pdxObject =
          gcnew PdxInstanceImpl(dataInput->GetBytes(dataInput->GetCursor(), len), len, typeId, true, dataInput->Cache);
...
Int32 PdxHelper::GetEnumValue(gc_ptr(String) enumClassName, gc_ptr(String) enumName, int hashcode,
                              gc_ptr(Cache) cache) {
  // in case app want different name
  enumClassName = cache->TypeRegistry->GetPdxTypeName(enumClassName);
  gc_ptr(EnumInfo) ei = gcnew EnumInfo(enumClassName, enumName, hashcode);
  return cache->GetPdxTypeRegistry()->GetEnumValue(ei);
}

gc_ptr(Object) PdxHelper::GetEnum(int enumId, gc_ptr(Cache) cache) {
  gc_ptr(EnumInfo) ei = cache->GetPdxTypeRegistry()->GetEnum(enumId);
...
Int32 PdxHelper::ReadByte(System::Byte* offsetPosition) { return (Int32)offsetPosition[0]; }
...
void PdxHelper::WriteByte(System::Byte* offsetPosition, Int32 value) { offsetPosition[0] = (Byte)value; }
