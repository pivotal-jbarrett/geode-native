...
ref class PdxWriterWithTypeCollector : public PdxLocalWriter {
 private:
  gc_ptr(System::Collections::Generic::List<Int32>) m_offsets;
  void initialize();

 public:
  PdxWriterWithTypeCollector(gc_ptr(DataOutput) dataOutput, gc_ptr(String) pdxClassName)
  ...
  property gc_ptr(PdxType) PdxLocalType {
    gc_ptr(PdxType) get() { return m_pdxType; }
  }
  ...
  virtual gc_ptr(IPdxWriter) WriteBoolean(gc_ptr(String) fieldName, Boolean value) override;
  ...
  virtual gc_ptr(IPdxWriter) WriteInt(gc_ptr(String) fieldName, Int32 value) override;
  ...
  virtual gc_ptr(IPdxWriter) MarkIdentityField(gc_ptr(String) fieldName) override;
