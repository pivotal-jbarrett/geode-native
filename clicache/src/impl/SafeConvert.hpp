...
inline static gc_ptr(Apache::Geode::Client::ISerializable)
  ...
  if (auto managedDataSerializable =
          std::dynamic_pointer_cast<native::ManagedCacheableKeyGeneric>(serializableObject)) {
    return managedDataSerializable->ptr();
  } else if (auto managedCacheableDeltaGeneric =
                 std::dynamic_pointer_cast<native::ManagedCacheableDeltaGeneric>(serializableObject)) {
    return dynamic_cast<gc_ptr(Apache::Geode::Client::IDataSerializable)>(managedCacheableDeltaGeneric->ptr());
  ...
  } else if (auto managedPrimitive =
                 std::dynamic_pointer_cast<native::ManagedDataSerializablePrimitive>(serializableObject)) {
    ...
    if (auto wrapperMethod = TypeRegistry::GetDataSerializablePrimitiveWrapperDelegateForDsCode(
            static_cast<int8_t>(primitive->getDsCode()))) {
      return wrapperMethod(primitive);
    }
  } else if (auto dataSerializableFixedId =
                 std::dynamic_pointer_cast<native::ManagedDataSerializableFixedId>(serializableObject)) {
    return dataSerializableFixedId->ptr();
  } else if (auto dataSerializableInternal =
                 std::dynamic_pointer_cast<native::ManagedDataSerializableInternal>(serializableObject)) {
...
inline static native::Serializable* GetNativeWrapperForManagedIDataSerializable(gc_ptr(IDataSerializable) mg_obj) {
  if (mg_obj == nullptr) return __nullptr;

  if (auto sDelta = dynamic_cast<gc_ptr(Apache::Geode::Client::IDelta)>(mg_obj)) {
...
inline static NativeType* GetNativePtr(gc_ptr(ManagedType) mg_obj) {
...
inline static native::Serializable* GetNativeWrapperForManagedObject(gc_ptr(Object) managedObject) {
  if (managedObject == nullptr) return __nullptr;

  if (auto dataSerializablePrimitive = dynamic_cast<gc_ptr(IDataSerializablePrimitive)>(managedObject)) {
    return new native::ManagedDataSerializablePrimitive(dataSerializablePrimitive);
  } else if (auto dataSerializable = dynamic_cast<gc_ptr(IDataSerializable)>(managedObject)) {
    return GetNativeWrapperForManagedIDataSerializable(dataSerializable);
  } else if (auto pdxSerializable = dynamic_cast<gc_ptr(IPdxSerializable)>(managedObject)) {
    return new native::PdxManagedCacheableKey(pdxSerializable);
  } else if (auto dataSerializableFixedId = dynamic_cast<gc_ptr(IDataSerializableFixedId)>(managedObject)) {
    return new native::ManagedDataSerializableFixedId(dataSerializableFixedId);
  } else if (auto dataSerializableInternal = dynamic_cast<gc_ptr(IDataSerializableInternal)>(managedObject)) {
...
inline static gc_ptr(IPdxSerializable) SafeUMSerializablePDXConvert(std::shared_ptr<native::Serializable> obj) {
  if (auto mg_obj = std::dynamic_pointer_cast<native::PdxManagedCacheableKey>(obj)) return mg_obj->ptr();
...
/// </summary>
GENERIC(class TKey)
inline static gc_ptr(Client::ICacheableKey) SafeGenericUMKeyConvert(std::shared_ptr<native::CacheableKey> obj) {
  ...
  if (auto mg_obj = std::dynamic_pointer_cast<native::ManagedCacheableKeyGeneric>(obj)) {
    return (gc_ptr(Client::ICacheableKey))mg_obj->ptr();
  }

  if (auto primitive = std::dynamic_pointer_cast<native::DataSerializablePrimitive>(obj)) {
    auto wrapperMethod =
        TypeRegistry::GetDataSerializablePrimitiveWrapperDelegateForDsCode(static_cast<int8_t>(primitive->getDsCode()));
    if (wrapperMethod != nullptr) {
      return (gc_ptr(Client::ICacheableKey))wrapperMethod(primitive);
    }
  }
...
template <typename NativeType, typename ManagedType>
inline static NativeType* GetNativePtr2(gc_ptr(ManagedType) mg_obj) {
