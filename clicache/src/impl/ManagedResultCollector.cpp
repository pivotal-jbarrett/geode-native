  ...
  try {
    ...
    if (typeInst != nullptr) {
      ...
      if (mInfo != nullptr) {
        ...
        if (userptr == nullptr) {
          std::string ex_str =
              "ManagedResultCollector: Could not create "
    ...
    std::string ex_str =
        "ManagedResultCollector: Got an exception while "
...
void ManagedResultCollectorGeneric::addResult(const std::shared_ptr<Cacheable>& result) {
  try {
    gc_ptr(Object) rs = Apache::Geode::Client::TypeRegistry::GetManagedValueGeneric<gc_ptr(Object)>(result);
    m_managedptr->AddResult(rs);
  } catch (gc_ptr(Apache::Geode::Client::GeodeException) ex) {
    ex->ThrowNative();
  } catch (gc_ptr(System::Exception) ex) {
    std::string ex_str =
        "ManagedResultCollector: Got an exception in"
        "addResult: " +
        marshal_as<std::string>(ex->ToString());
    throw apache::geode::client::IllegalArgumentException(ex_str);
  }
...
std::shared_ptr<CacheableVector> ManagedResultCollectorGeneric::getResult(std::chrono::milliseconds timeout) {
  try {
    ...
    //  valueptr(Apache::Geode::Client::Serializable::GetUnmanagedValueGeneric<gc_ptr(ISerializable)>(rs[ index]));
