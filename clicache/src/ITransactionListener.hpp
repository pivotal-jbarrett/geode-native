...
PUBLIC interface class ITransactionListener {
 public:
  ...
  void AfterCommit(gc_ptr(TransactionEvent<TKey, TValue>) te);
  ...
  /// <param name="te">the transaction event</param>
  /// <seealso cref="CacheTransactionManager.Commit" />
  void AfterFailedCommit(gc_ptr(TransactionEvent<TKey, TValue>) te);
  ...
  void AfterRollback(gc_ptr(TransactionEvent<TKey, TValue>) te);
