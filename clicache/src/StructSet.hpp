...
/// </summary>
GENERIC(class TResult)
PUBLIC ref class StructSet sealed : public ICqResults<TResult> {
 public:
  /// <summary>
  ...
  /// </returns>
  virtual property size_t Size { virtual size_t get(); }
  ...
  virtual property TResult GFINDEXER(size_t) { virtual TResult get(size_t index); }
  ...
  /// </exception>
  int32_t GetFieldIndex(gc_ptr(String) fieldName);
  ...
  /// </returns>
  gc_ptr(String) GetFieldName(int32_t index);

  // Region: IEnumerable<gc_ptr(ISerializable)> Members
  ...
  // End Region: IEnumerable<gc_ptr(ISerializable)> Members
  ...
  internal :
      ...
      /// </returns>
      inline static gc_ptr(StructSet<TResult>) Create(std::shared_ptr<native::StructSet> nativeptr) {
    return __nullptr == nativeptr ? nullptr : gcnew StructSet<TResult>(nativeptr);
  }
 ...
 private:
  ...
  gc_ptr(native_shared_ptr<native::StructSet>) m_nativeptr;
};
