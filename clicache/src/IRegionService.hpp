...
PUBLIC interface class IRegionService {
 public:
  ...
  /// If RegionService is created from <see cref="Cache.CreateAuthenticatedView" />, then it clears user related
  /// security data.
  /// </summary>
  /// <remarks>
  ...
  GENERIC(class TKey, class TValue)
  gc_ptr(IRegion<TKey, TValue>) GetRegion(gc_ptr(String) name);
  ...
  /// </remarks>
  gc_ptr(Client::QueryService) GetQueryService();
  /// <summary>
  ...
  GENERIC(class TKey, class TValue)
  array<gc_ptr(IRegion<TKey, TValue>)> ^ RootRegions();
  ...
  /// @return the factory
  /// </summary>
  gc_ptr(IPdxInstanceFactory) CreatePdxInstanceFactory(gc_ptr(String) className);
};
}  // namespace Client
}  // namespace Geode
}  // namespace Apache
