...
/// </remarks>
delegate gc_ptr(GeodeException)
    CreateException2(const apache::geode::client::Exception& nativeEx, gc_ptr(System::Exception) innerException);

/// <summary>
/// The base exception class of all managed Geode exceptions.
/// </summary>
[Serializable] PUBLIC ref class GeodeException : public System::Exception {
 private:
  /// <summary>
  /// Prefix for distiguishing managed system exceptions
  /// </summary>
  literal gc_ptr(String) MgSysExPrefix = "GFCLI_EXCEPTION:";
  ...
  /// </returns>
  static gc_ptr(Exception) Get(const apache::geode::client::Exception& nativeEx);
  ...
  inline static gc_ptr(String) GetStackTrace(const apache::geode::client::Exception& nativeEx) {
    return marshal_as<gc_ptr(String)>(nativeEx.getStackTrace());
  }
  ...
  /// </remarks>
  inline static std::shared_ptr<apache::geode::client::Exception> GetNative(gc_ptr(Exception) ex) {
    if (ex != nullptr) {
      gc_ptr(GeodeException) gfEx = dynamic_cast<gc_ptr(GeodeException)>(ex);
      if (gfEx != nullptr) {
        ...
        return std::make_shared<apache::geode::client::Exception>(
            marshal_as<std::string>(MgSysExPrefix + ex->ToString()) + (cause ? cause->getMessage() : ""));
      }
    }
    return nullptr;
  }
  ...
  virtual std::shared_ptr<apache::geode::client::Exception> GetNative() {
    ...
    return std::make_shared<apache::geode::client::Exception>(
        marshal_as<std::string>(this->Message + ": " + this->StackTrace) + (cause ? cause->getMessage() : ""));
  }
  ...
  /// original stacktrace is appended to the message of the exception.
  /// </remarks>
  inline static void ThrowNative(gc_ptr(Exception) ex) {
    if (ex != nullptr) {
      std::shared_ptr<apache::geode::client::Exception> cause;
      if (ex->InnerException != nullptr) {
        cause = GeodeException::GetNative(ex->InnerException);
      }
      throw apache::geode::client::Exception(marshal_as<std::string>(MgSysExPrefix + ex->ToString()) +
  ...
  /// <c>GeodeException</c>.
  /// </summary>
  inline void ThrowNative() { throw GetNative(); }

 public:
  /// <summary>
  /// Default constructor.
  /// </summary>
  inline GeodeException() : Exception() {}

  /// <summary>
  /// Constructor to create an exception object with the given message.
  /// </summary>
  /// <param name="message">The exception message.</param>
  inline GeodeException(gc_ptr(String) message) : Exception(message) {}
  ...
  inline GeodeException(gc_ptr(String) message, gc_ptr(System::Exception) innerException)
 ...
 protected:
  ...
  /// </param>
  inline GeodeException(gc_ptr(SerializationInfo) info, StreamingContext context) : Exception(info, context) {}
};
