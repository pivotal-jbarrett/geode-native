...
PUBLIC ref class PoolFactory sealed {
 public:
  ...
  /// </exception>
  gc_ptr(PoolFactory) SetFreeConnectionTimeout(TimeSpan connectionTimeout);
  ...
  /// is less than 0.
  /// </exception>
  gc_ptr(PoolFactory) SetLoadConditioningInterval(TimeSpan loadConditioningInterval);
  ...
  /// is less than or equal to 0.
  /// </exception>
  gc_ptr(PoolFactory) SetSocketBufferSize(Int32 bufferSize);
  ...
  /// is less than or equal to 0.
  /// </exception>
  gc_ptr(PoolFactory) SetReadTimeout(TimeSpan timeout);
  ...
  /// </exception>
  gc_ptr(PoolFactory) SetMinConnections(Int32 minConnections);
  ...
  /// </exception>
  gc_ptr(PoolFactory) SetMaxConnections(Int32 maxConnections);
  ...
  /// </exception>
  gc_ptr(PoolFactory) SetIdleTimeout(TimeSpan idleTimeout);
  ...
  /// throws IllegalArgumentException if idleTimout is less than 0.
  /// </exception>
  gc_ptr(PoolFactory) SetRetryAttempts(Int32 retryAttempts);
  ...
  /// </exception>
  gc_ptr(PoolFactory) SetPingInterval(TimeSpan pingInterval);
  ...
  /// </returns>
  gc_ptr(PoolFactory) SetUpdateLocatorListInterval(TimeSpan updateLocatorListInterval);
  ...
  /// is less than 0.
  /// </exception>
  gc_ptr(PoolFactory) SetStatisticInterval(TimeSpan statisticInterval);
  ...
  /// </param>
  gc_ptr(PoolFactory) SetServerGroup(gc_ptr(String) group);
  ...
  /// </exception>
  gc_ptr(PoolFactory) AddLocator(gc_ptr(String) host, Int32 port);
  ...
  /// </exception>
  gc_ptr(PoolFactory) AddServer(gc_ptr(String) host, Int32 port);
  ...
  /// port the port that the server is listening on.
  /// </param>
  gc_ptr(PoolFactory) SetSniProxy(gc_ptr(String) hostname, Int32 port);
  ...
  /// </remarks>
  gc_ptr(PoolFactory) SetSubscriptionEnabled(Boolean enabled);
  ...
  /// </remarks>
  gc_ptr(PoolFactory) SetPRSingleHopEnabled(Boolean enabled);
  ...
  /// </exception>
  gc_ptr(PoolFactory) SetSubscriptionRedundancy(Int32 redundancy);
  ...
  /// </exception>
  gc_ptr(PoolFactory) SetSubscriptionMessageTrackingTimeout(TimeSpan messageTrackingTimeout);
  ...
  /// </exception>
  gc_ptr(PoolFactory) SetSubscriptionAckInterval(TimeSpan ackInterval);
  ...
  /// connections down.
  /// </remarks>
  gc_ptr(PoolFactory) SetThreadLocalConnections(Boolean enabled);

  /// <summary>
  /// Sets whether pool is in multiuser mode
  /// If its in multiuser mode then app needs to get instance of cache from pool.getCache("creds"), to do the operations
  /// on cache.
  /// </summary>
  /// <param>
  /// multiuserAuthentication should be true/false. Default value is false;
  /// </param>
  gc_ptr(PoolFactory) SetMultiuserAuthentication(bool multiuserAuthentication);

  /// <summary>
  /// Resets the configuration of this factory to its defaults.
  /// </summary>
  gc_ptr(PoolFactory) Reset();
  ...
  /// </exception>
  gc_ptr(Pool) Create(gc_ptr(String) name);
  ...
  /// </returns>
  inline static gc_ptr(PoolFactory) Create(std::shared_ptr<native::PoolFactory> nativeptr) {
    return __nullptr == nativeptr ? nullptr : gcnew PoolFactory(nativeptr);
  }
 ...
 private:
  ...
  gc_ptr(native_shared_ptr<native::PoolFactory>) m_nativeptr;
};
}  // namespace Client
}  // namespace Geode
}  // namespace Apache
