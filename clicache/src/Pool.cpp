...
gc_ptr(String) Pool::SniProxyHost::get() {
  try {
    return marshal_as<gc_ptr(String)>(m_nativeptr->get()->getSniProxyHost());
  } finally {
    GC::KeepAlive(m_nativeptr);
  }
}
...
gc_ptr(String) Pool::ServerGroup::get() {
  try {
    return marshal_as<gc_ptr(String)>(m_nativeptr->get()->getServerGroup());
  } finally {
    GC::KeepAlive(m_nativeptr);
  }
}
...
gc_ptr(array<String ^>) Pool::Locators::get() {
  try {
    ...
    if (length > 0) {
      auto result = gcnew array<gc_ptr(String)>(length);
      for (int item = 0; item < length; item++) {
        result[item] = marshal_as<gc_ptr(String)>((*locators)[item]->toString());
      }
      return result;
    } else {
      return nullptr;
...
gc_ptr(QueryService) Pool::GetQueryService() {
