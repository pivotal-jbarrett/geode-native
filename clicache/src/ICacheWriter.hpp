...
PUBLIC interface class ICacheWriter {
 public:
  ...
  bool BeforeUpdate(gc_ptr(EntryEvent<TKey, TValue>) ev);
  ...
  /// <seealso cref="Region.Put" />
  /// <seealso cref="Region.Get" />
  bool BeforeCreate(gc_ptr(EntryEvent<TKey, TValue>) ev);
  ...
  bool BeforeDestroy(gc_ptr(EntryEvent<TKey, TValue>) ev);

  /// <summary>
  /// Called before this region is cleared.
  /// </summary>
  bool BeforeRegionClear(gc_ptr(RegionEvent<TKey, TValue>) ev);
  ...
  bool BeforeRegionDestroy(gc_ptr(RegionEvent<TKey, TValue>) ev);
