...
PUBLIC ref class DataInput sealed {
 public:
  ...
  /// </exception>
  DataInput(gc_ptr(array<Byte>) buffer, gc_ptr(Cache) cache);
  ...
  /// if the buffer is null
  /// </exception>
  DataInput(gc_ptr(array<Byte>) buffer, size_t len, gc_ptr(Cache) cache);
  ...
  void ReadDictionary(gc_ptr(System::Collections::IDictionary) dictionary);
  ...
  void ReadCollection(gc_ptr(System::Collections::IList) list);

  /// <summary>
  /// Read a char array from the stream.
  /// </summary>
  gc_ptr(array<Char>) ReadCharArray();

  /// <summary>
  /// Read a bool array from the stream.
  /// </summary>
  gc_ptr(array<bool>) ReadBooleanArray();

  /// <summary>
  /// Read a short int array from the stream.
  /// </summary>
  gc_ptr(array<Int16>) ReadShortArray();

  /// <summary>
  /// Read a int array from the stream.
  /// </summary>
  gc_ptr(array<Int32>) ReadIntArray();

  /// <summary>
  /// Read a long array from the stream.
  /// </summary>
  gc_ptr(array<Int64>) ReadLongArray();

  /// <summary>
  /// Read a float array from the stream.
  /// </summary>
  gc_ptr(array<float>) ReadFloatArray();

  /// <summary>
  /// Read a double array from the stream.
  /// </summary>
  gc_ptr(array<double>) ReadDoubleArray();
  ...
  /// Read a string after java-modified UTF-8 decoding from the stream.
  /// </summary>
  gc_ptr(String) ReadUTFHuge();

  /// <summary>
  /// Read a ASCII string from the stream. Where size is more than gc_ptr(2)16-1
  /// </summary>
  gc_ptr(String) ReadASCIIHuge();

  native::Pool* GetPool();

  gc_ptr(Object) ReadDotNetTypes(int8_t typeId);

  /// <summary>
  /// Get the count of bytes that have been read from the stream, for internal use only.
  /// </summary>
  property size_t BytesReadInternally { size_t get(); }

  void ReadObject(bool % obj) { obj = ReadBoolean(); }

  void ReadObject(Byte % obj) { obj = ReadByte(); }

  void ReadObject(Char % obj) { obj = (Char)ReadUInt16(); }
  ...
  gc_ptr(System::Collections::Hashtable) ReadHashtable() {
    int len = this->ReadArrayLen();

    if (len == -1)
      return nullptr;
    else {
      gc_ptr(System::Collections::Hashtable) dict = gcnew System::Collections::Hashtable();
      for (int i = 0; i < len; i++) {
  ...
  void ReadObject(Double % obj) { obj = ReadDouble(); }

  void ReadObject(Single % obj) { obj = ReadFloat(); }

  void ReadObject(System::Int16 % obj) { obj = ReadInt16(); }

  void ReadObject(System::Int32 % obj) { obj = ReadInt32(); }

  void ReadObject(System::Int64 % obj) { obj = ReadInt64(); }

  void ReadObject(gc_ptr(array<SByte>) % obj) { obj = ReadSBytes(); }
  ...
  void ReadObject(gc_ptr(array<mType>) % objArray) {
    ...
    if (len == -1) {
      ...
      if (len > 0) {
        for (int i = 0; i < len; i++) {
          ...
          if (obj != nullptr)
            ret[i] = static_cast<gc_ptr(String)>(obj);
          else
            ret[i] = nullptr;
        }
      }
  ...
  System::Byte* GetCursor() { return m_buffer + m_cursor; }
  ...
  void AdvanceCursorPdx(int offset) { m_cursor += offset; }

  void RewindCursorPdx(int rewind) { m_cursor = 0; }

  void ResetAndAdvanceCursorPdx(int offset) { m_cursor = offset; }
  ...
  inline gc_ptr(array<Byte>) ReadReverseBytesOnly(int len);
  ...
  gc_ptr(String) DecodeBytes(int length) {
    // gc_ptr(array<Char>) output = gcnew array<Char>(length);

    if (m_forStringDecode->Length < length) m_forStringDecode = gcnew array<Char>(length);
    // index input[]
    int i = 0;
    // index output[]
    int j = 0;
    ...
    gc_ptr(String) str = gcnew String(m_forStringDecode, 0, j);
    return str;
  }

  void CheckBufferSize(int size);

  gc_ptr(Object) ReadInternalObject();

  gc_ptr(DataInput) GetClone();
  ...
  DataInput(System::Byte* buffer, size_t size, gc_ptr(Apache::Geode::Client::Cache) cache);

  bool IsManagedObject() { return m_isManagedObject; }

  int GetPdxBytes() { return static_cast<int>(m_bufferLength); }
 ...
 private:
  ...
  gc_ptr(native_unique_ptr<System::Byte[]>) m_ownedBuffer;
  size_t m_bufferLength;
  size_t m_cursor;
  bool m_isManagedObject;
  gc_ptr(array<Char>) m_forStringDecode;

  gc_ptr(native_conditional_unique_ptr<native::DataInput>) m_nativeptr;
};
}  // namespace Client
}  // namespace Geode
}  // namespace Apache
