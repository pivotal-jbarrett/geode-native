...
class ManagedDataSerializableHandler : public DataSerializableHandler {
 public:
  ...
  void serialize(const std::shared_ptr<DataSerializable>& dataSerializable, DataOutput& dataOutput,
                 bool isDelta) const override {
    gc_ptr(Managed::ISerializable) data = Managed::SafeUMSerializableConvertGeneric(dataSerializable);
  ...
  std::shared_ptr<DataSerializable> deserialize(DataInput& dataInput, DSCode typeId) const override {
    try {
      ...
      auto managedDataSerializable = (gc_ptr(Apache::Geode::Client::IDataSerializable))createType();
