...
DataInput::DataInput(System::Byte* buffer, size_t size, gc_ptr(Apache::Geode::Client::Cache) cache) {
...
DataInput::DataInput(gc_ptr(array<Byte>) buffer, gc_ptr(Apache::Geode::Client::Cache) cache) {
  m_ispdxDesrialization = false;
  m_isRootObjectPdx = false;
  m_cache = cache;
...
DataInput::DataInput(gc_ptr(array<Byte>) buffer, size_t len, gc_ptr(Apache::Geode::Client::Cache) cache) {
  m_ispdxDesrialization = false;
  m_isRootObjectPdx = false;
  m_cache = cache;
  if (buffer != nullptr) {
    if (len == 0 || (System::Int32)len > buffer->Length) {
      throw gcnew IllegalArgumentException(
          String::Format("DataInput.ctor(): given length {0} is zero or greater than "
                         "size of buffer {1}",
                         len, buffer->Length));
    }
    _GF_MG_EXCEPTION_TRY2
...
void DataInput::CheckBufferSize(int size) {
  if ((unsigned int)(m_cursor + size) > m_bufferLength) {
    Log::Debug("DataInput::CheckBufferSize m_cursor:" + m_cursor + " size:" + size +
               " m_bufferLength:" + m_bufferLength);
    throw gcnew OutOfRangeException("DataInput: attempt to read beyond buffer");
  }
}

gc_ptr(DataInput) DataInput::GetClone() { return gcnew DataInput(m_buffer, m_bufferLength, m_cache); }
...
gc_ptr(array<Byte>) DataInput::ReadBytes() {
  System::Int32 length;
  length = ReadArrayLen();
  ...
  if (length >= 0) {
    if (length == 0)
      ...
      gc_ptr(array<Byte>) bytes = ReadBytesOnly(length);
      return bytes;
    }
  }
  return nullptr;
...
gc_ptr(array<SByte>) DataInput::ReadSBytes() {
  System::Int32 length;
  length = ReadArrayLen();
  ...
  if (length > -1) {
    if (length == 0)
      ...
      gc_ptr(array<SByte>) bytes = ReadSBytesOnly(length);
      return bytes;
    }
  }
  return nullptr;
...
gc_ptr(array<Byte>) DataInput::ReadBytesOnly(System::UInt32 len) {
  if (len > 0) {
    CheckBufferSize(len);
    gc_ptr(array<Byte>) bytes = gcnew array<Byte>(len);
...
void DataInput::ReadBytesOnly(gc_ptr(array<Byte>) buffer, int offset, int count) {
  if (count > 0) {
...
gc_ptr(array<SByte>) DataInput::ReadSBytesOnly(System::UInt32 len) {
  if (len > 0) {
    CheckBufferSize(len);
    gc_ptr(array<SByte>) bytes = gcnew array<SByte>(len);

    for (unsigned int i = 0; i < len; i++) bytes[i] = (SByte)m_buffer[m_cursor++];
...
gc_ptr(array<Byte>) DataInput::ReadReverseBytesOnly(int len) {
  CheckBufferSize(len);
...
gc_ptr(String) DataInput::ReadUTF() {
  int length = ReadUInt16();
  CheckBufferSize(length);
  gc_ptr(String) str = DecodeBytes(length);
  return str;
}

gc_ptr(String) DataInput::ReadUTFHuge() {
  int length = ReadUInt32();
  CheckBufferSize(length);

  gc_ptr(array<Char>) chArray = gcnew array<Char>(length);
  ...
  gc_ptr(String) str = gcnew String(chArray);

  return str;
}

gc_ptr(String) DataInput::ReadASCIIHuge() {
...
gc_ptr(Object) DataInput::ReadObject() { return ReadInternalObject(); }

/*	gc_ptr(Object) DataInput::ReadGenericObject( )
  {
  return ReadInternalGenericObject();
  }*/

gc_ptr(Object) DataInput::ReadDotNetTypes(int8_t typeId) {
  switch (static_cast<apache::geode::client::internal::DSCode>(typeId)) {
    ...
    case apache::geode::client::internal::DSCode::CacheableASCIIString: {
      /*	gc_ptr(CacheableString) cs =
        static_cast<gc_ptr(CacheableString)>(CacheableString::CreateDeserializable()); cs->FromData(this); return
        cs->Value;*/
      return ReadUTF();
    }
    case apache::geode::client::internal::DSCode::CacheableASCIIStringHuge: {
      /*gc_ptr(CacheableString) cs =
      static_cast<gc_ptr(CacheableString)>(CacheableString::createDeserializableHuge()); cs->FromData(this);
      return cs->Value;*/
      return ReadASCIIHuge();
    }
    case apache::geode::client::internal::DSCode::CacheableString: {
      /*gc_ptr(CacheableString) cs =
      static_cast<gc_ptr(CacheableString)>(CacheableString::createUTFDeserializable()); cs->FromData(this); return
      cs->Value;*/
      return ReadUTF();
    }
    case apache::geode::client::internal::DSCode::CacheableStringHuge: {
      ...
      /*gc_ptr(CacheableString) cs =
      static_cast<gc_ptr(CacheableString)>(CacheableString::createUTFDeserializableHuge()); cs->FromData(this);
      return cs->Value;*/
      return ReadUTFHuge();
    }
    ...
    case apache::geode::client::internal::DSCode::CacheableDate: {
      gc_ptr(CacheableDate) cd = CacheableDate::Create();
      cd->FromData(this);
      return cd->Value;
    }
    ...
    case apache::geode::client::internal::DSCode::CacheableIdentityHashMap: {
      gc_ptr(CacheableIdentityHashMap) chm =
          static_cast<gc_ptr(CacheableIdentityHashMap)>(CacheableIdentityHashMap::CreateDeserializable());
      chm->FromData(this);
      return chm->Value;
    }
    case apache::geode::client::internal::DSCode::CacheableVector: {
      /*gc_ptr(CacheableVector) cv =
      static_cast<gc_ptr(CacheableVector)>(CacheableVector::CreateDeserializable()); cv->FromData(this); return
      cv->Value;*/
      int len = ReadArrayLen();
      gc_ptr(System::Collections::ArrayList) retA = gcnew System::Collections::ArrayList(len);
    ...
    case apache::geode::client::internal::DSCode::CacheableStack: {
      gc_ptr(CacheableStack) cv = static_cast<gc_ptr(CacheableStack)>(CacheableStack::CreateDeserializable());
      cv->FromData(this);
      return cv->Value;
    }
    default:
...
gc_ptr(Object) DataInput::ReadInternalObject() {
  try {
    ...
    gc_ptr(TypeFactoryMethod) createType = nullptr;
    ...
    switch (static_cast<DSCode>(typeId)) {
      ...
      case DSCode::PDX: {
        ...
        gc_ptr(Object) ret = Internal::PdxHelper::DeserializePdx(
            this, false, CacheRegionHelper::getCacheImpl(m_cache->GetNative().get())->getSerializationRegistry().get());
        ...
        if (ret != nullptr) {
          if (auto pdxWrapper = dynamic_cast<gc_ptr(Apache::Geode::Client::PdxWrapper)>(ret)) {
      ...
      case DSCode::PDX_ENUM: {
        ...
        gc_ptr(Object) enumVal = Internal::PdxHelper::GetEnum(enumId, m_cache);
        return enumVal;
      }
    ...
    if (findinternal) {
      createType =
          m_cache->TypeRegistry->GetDataSerializableFixedTypeFactoryMethodForFixedId(static_cast<Int32>(compId));
    } else {
      createType = m_cache->TypeRegistry->GetManagedObjectFactory(compId);
      if (createType == nullptr) {
        gc_ptr(Object) retVal = ReadDotNetTypes(typeId);

        if (retVal != nullptr) return retVal;

        if (m_ispdxDesrialization &&
            static_cast<DSCode>(typeId) == DSCode::CacheableObjectArray) {  // object array and pdxSerialization
    ...
    gc_ptr(ISerializable) newObj = createType();

    if (auto dataSerializable = dynamic_cast<gc_ptr(IDataSerializablePrimitive)>(newObj)) {
      dataSerializable->FromData(this);
    } else if (auto dataSerializable = dynamic_cast<gc_ptr(IDataSerializable)>(newObj)) {
      dataSerializable->FromData(this);
    } else if (auto dataSerializableFixedId = dynamic_cast<gc_ptr(IDataSerializableFixedId)>(newObj)) {
...
gc_ptr(Object) DataInput::readDotNetObjectArray() {
  int len = ReadArrayLen();
  gc_ptr(String) className = nullptr;
  if (len >= 0) {
    ReadByte();  // ignore CLASS typeid
    className = (gc_ptr(String))ReadObject();
    className = m_cache->TypeRegistry->GetLocalTypeName(className);
    gc_ptr(System::Collections::IList) list = nullptr;
    if (len == 0) {
      list = (gc_ptr(System::Collections::IList))m_cache->TypeRegistry->GetArrayObject(className, len);
      return list;
    }
    // read first object

    gc_ptr(Object) ret = ReadObject();  // in case it returns pdxinstance or java.lang.object

    list = (gc_ptr(System::Collections::IList))m_cache->TypeRegistry->GetArrayObject(ret->GetType()->FullName, len);
...
void DataInput::ReadDictionary(gc_ptr(System::Collections::IDictionary) dict) {
...
void DataInput::ReadCollection(gc_ptr(System::Collections::IList) coll) {
  int len = ReadArrayLen();
  for (int i = 0; i < len; i++) {
    coll->Add(ReadObject());
  }
...
gc_ptr(array<Char>) DataInput::ReadCharArray() {
  gc_ptr(array<Char>) arr;
  this->ReadObject(arr);
  return arr;
}

gc_ptr(array<bool>) DataInput::ReadBooleanArray() {
  gc_ptr(array<bool>) arr;
  this->ReadObject(arr);
  return arr;
}

gc_ptr(array<Int16>) DataInput::ReadShortArray() {
  gc_ptr(array<Int16>) arr;
  this->ReadObject(arr);
  return arr;
}

gc_ptr(array<Int32>) DataInput::ReadIntArray() {
  gc_ptr(array<Int32>) arr;
  this->ReadObject(arr);
  return arr;
}

gc_ptr(array<Int64>) DataInput::ReadLongArray() {
  gc_ptr(array<Int64>) arr;
  this->ReadObject(arr);
  return arr;
}

gc_ptr(array<float>) DataInput::ReadFloatArray() {
  gc_ptr(array<float>) arr;
  this->ReadObject(arr);
  return arr;
}

gc_ptr(array<double>) DataInput::ReadDoubleArray() {
...
array<gc_ptr(array<Byte>)> ^ DataInput::ReadArrayOfByteArrays() {
  int len = ReadArrayLen();
  if (len >= 0) {
    array<gc_ptr(array<Byte>)> ^ retVal = gcnew array<gc_ptr(array<Byte>)>(len);
    for (int i = 0; i < len; i++) {
...
void DataInput::ReadObject(gc_ptr(array<UInt16>) % obj) {
  int len = ReadArrayLen();
  if (len >= 0) {
...
void DataInput::ReadObject(gc_ptr(array<UInt32>) % obj) {
  int len = ReadArrayLen();
  if (len >= 0) {
...
void DataInput::ReadObject(gc_ptr(array<UInt64>) % obj) {
  int len = ReadArrayLen();
  if (len >= 0) {
...
gc_ptr(String) DataInput::ReadString() {
